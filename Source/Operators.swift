...
public func <-> <T>(property: ControlProperty<T>, variable: Variable<T>) -> Disposable {
    ...
    let bindToVariable = property
        .subscribe(onNext: { n in
            ...
            guard let n = n as? Bool else {
                return
            }
            
            guard let variable = variable as? Variable<Bool> else {
                return
            }
...
public extension UIControl {
    public static func valuePublic<T, ControlType: UIControl>(_ control: ControlType, getter: @escaping (ControlType) -> T, setter: @escaping (ControlType, T) -> (), controlEvents: UIControlEvents) -> ControlProperty<T> {
        ...
        let values: Observable<T> = Observable.deferred { [weak control] in
            ...
            return existingSelf.rx.controlEvent(controlEvents).flatMap{
                _ in
